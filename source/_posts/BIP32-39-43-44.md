...
大致意思就是说，我们这里呢，是放置bitcoin改进建议的！在这里，我们对于任何一个意见都是需要大家达到共识的。

当然，如果真的发生了很让人争议的建议，我们会选择保守的方式。猥琐发育才是王道！

<!-- more -->

## BIP 规则

在 [BIP-001](https://github.com/bitcoin/bips/blob/master/bip-0001.mediawiki),规定了一些BIPS中提意见的规则。
...
* 32 提出了整个建议，从生成`Master Seed`(根种子)，以及包括如何创建子私钥，子公钥，扩展秘钥等。        
* 39 完善了建议，认为随机生成的根种子不好记录，所以我们先生成若干个(12-18)助记词       
* 43 在32提出了 目录分层为 `m/0/0 ->` ，认为应该设置为 `m/purpose/*` 更容易让人理解 (其中m为Master Seed)
* 44 在43提出的基础上 更加完善了 结构  `m / purpose' / coin_type' / account' / change / address_index`
...
在这里的时候，无论是我们直接生成`Master Seed`还是通过`BIP39` 来生成 `Master Seed`.
我们都已经有随机种子了,记下来让我们来看看下面的过程吧。
...
索引号个数为 $2^{31}$ 次方，每个父级密钥能推导出该数目一半的子密钥 (
索引号从 0x00 到 0x7fffffff (0 to $2^{21}-1$) 会生成正常的密钥；索引号从 0x80000000 到 0xffffffff 会生成增强密钥）。CKD 采用不可逆的 HMAC-SHA512 不可逆加密算法，子密钥不能向上推导出父密钥、同时也不能水平推导出同一级的密钥。
